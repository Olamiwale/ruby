    ...
    const fetchPayments = async () => {
      try {
        ...
        if (axios.isAxiosError(err)) {
          setPaymentsError(err.response?.data?.message || "Could not load payment history.");
        } else {
      ...
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className="bg-gray-900 rounded-2xl px-8 py-8 flex flex-wrap items-center gap-5">
          ...
          {(user as any).createdAt && (
            <div className="text-right">
              ...
              <p className="text-sm text-gray-300 font-medium">{formatDate((user as any).createdAt)}</p>
            </div>
          )}
        </div>
                ...
                >
                  ...
                  {isExpanded && items.length === 0 && (
                    <div className="px-7 pb-4 bg-stone-50 border-t border-stone-100">
                      <p className="text-xs text-gray-400 py-3 italic">No item details recorded for this order.</p>
                    </div>
                  )}
                </div>
              );
        ...
        <div className="flex justify-center gap-3 pt-2">
          ...
          <button
            ...
            className="bg-gray-900 text-white text-xs px-7 py-2.5 rounded-lg hover:bg-gray-700 transition-colors"
          >
            Sign Out
          </button>
        </div>
